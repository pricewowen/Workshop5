...
public class BakeryLocationsController {
    ...
    private void showBakeryDialog(Bakery existing) {
        ...
        saveBtn.addEventFilter(javafx.event.ActionEvent.ACTION, event -> {
            ...
            String err = validateDialog(tfName, tfPhone, tfEmail, tfLine1, tfLine2, tfCity, prov, tfPostal);
            if (err != null) {
        ...
        dialog.showAndWait().ifPresent(result -> {
            ...
            Address addr = new Address(
                    existingAddr != null ? existingAddr.getAddressId() : 0,
                    tfLine1.getText(), tfLine2.getText(), tfCity.getText(),
                    prov != null ? prov.getCode() : "", tfPostal.getText()
            );
            Bakery b = isNew
                    ? new Bakery(0, addr, tfName.getText(), tfPhone.getText(), tfEmail.getText())
                    : existing;
            if (!isNew) {
                b.setBakeryName(tfName.getText());
                b.setBakeryPhone(tfPhone.getText());
                b.setBakeryEmail(tfEmail.getText());
                b.setAddress(addr);
            }
            try {
                int savedBakeryId;
    ...
    private void handleDeleteBakery(Bakery b) {
        ...
        try {
            BakeryApi.delete(b.getBakeryId());
            ...
            refreshTable();
            if (lblStatus != null) lblStatus.setText("Deleted location #" + b.getBakeryId());
        } catch (Exception ex) {
    ...
    private void applyCapFirstLetter(TextField tf) {
        tf.textProperty().addListener((obs, oldText, newText) -> {
            ...
            String cap = newText.substring(0, 1).toUpperCase() + newText.substring(1);
